...
/*   Updated: 2024/04/11 18:28:54 by marirodr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_create_window(t_game *game)
{
	...
	texture->enemy_right = mlx_load_png("img/dog_1.png");
	if (!texture->player || !texture->floor || !texture->wall
		...
		|| !texture->player_s || !texture->enemy_right)
		ft_error(MLX_FAIL);
...
void	ft_texture_to_image(t_game *game, t_texture *texture)
{
	...
	game->enemy_right_img \
	= mlx_texture_to_image(game->mlx, texture->enemy_right);
}
...
void	ft_delete_texture(t_texture *texture)
{
	...
	mlx_delete_texture(texture->enemy_right);
	free(texture);
}
	...
	while (game->map[++y])
	{
		...
		while (game->map[y][++x])
		{
			...
			else if (game->map[y][x] == 'E')
			{
				...
				ft_render_enemy(game, y, x, &enemy);
		}
	}
}
